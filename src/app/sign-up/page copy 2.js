...
// ─── CONFIG ────────────────────────────────────────────────────────────────────
const GOOGLE_CLIENT_ID  = "1709448650-s2d63dqb81qgli9at6vnq391fb7csr9a.apps.googleusercontent.com";
const AUTH_API          = "/api/auth";
const EMAIL_CHECK_API   = `${AUTH_API}?apiType=checkGoogleEmail`;
const GOOGLE_SIGNUP_API = `${AUTH_API}?apiType=googleSignup`;
  ...
  const processCredential = useCallback(async (credential) => {
    ...
    try {
      ...
      if (checkData.data?.accountExists) {
        ...
        setGoogleStage("done");
        localStorage.setItem("validationEmail", user.emailAddress);
